...
// Institution and Roll Based Staffs Simple List -----------------------------------------------
exports.InstitutionAndRollBased_StaffsSimpleList= function(req, res) {
   var CryptoBytes  = CryptoJS.AES.decrypt(req.body.Info, 'SecretKeyIn@123');
   var ReceivingData = JSON.parse(CryptoBytes.toString(CryptoJS.enc.Utf8));
   ...
   } else if(!ReceivingData.Roll || ReceivingData.Roll === ''){
      res.status(400).send({Status: false, Message: "Staff Roll can not be empty" });
   } else {
      StaffsModel.StaffsSchema
         .find({'If_Deleted' : false, UserManagement_Linked: false, Institution: mongoose.Types.ObjectId(ReceivingData.Institution), StaffRole: ReceivingData.Roll}, { Department: 1, Name: 1 }, {sort: { updatedAt: -1 }})
         .populate({ path: 'Department', select: 'Department' })
         .exec(function(err, result) {
            if(err) {
               ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'Staffs Find Query Error', 'Staffs.controller.js', err);
...
// Department Based Staff's  List -----------------------------------------------
exports.DepartmentBased_StaffsList= function(req, res) {
   var CryptoBytes  = CryptoJS.AES.decrypt(req.body.Info, 'SecretKeyIn@123');
   var ReceivingData = JSON.parse(CryptoBytes.toString(CryptoJS.enc.Utf8));
