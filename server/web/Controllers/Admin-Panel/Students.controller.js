...
exports.DepartmentBased_StudentsList= function(req, res) {
   var CryptoBytes  = CryptoJS.AES.decrypt(req.body.Info, 'SecretKeyIn@123');
   var ReceivingData = JSON.parse(CryptoBytes.toString(CryptoJS.enc.Utf8));
   ...
   if (!ReceivingData.User_Id || ReceivingData.User_Id === ''  ) {
      ...
      CurrentSemestersModel.CurrentSemestersSchema
         .find({'If_Deleted' : false, Institution: mongoose.Types.ObjectId(ReceivingData.Institution), Department: mongoose.Types.ObjectId(ReceivingData.Department)}, {Yearly_Badge: 1, Semester: 1}, {sort: { updatedAt: -1 }})
         .exec(function(err, result) {
            if(err) {
               ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'Students Find Query Error', 'Students.controller.js', err);
               res.status(417).send({status: false, Message: "Some error occurred while Find The Students !."});
