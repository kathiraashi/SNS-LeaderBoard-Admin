...
@Component({
  selector: 'app-model-achievement-type',
  templateUrl: './model-achievement-type.component.html',
  styleUrls: ['./model-achievement-type.component.css']
})
export class ModelAchievementTypeComponent implements OnInit {
   ...
   ngOnInit() {
      ...
      // If Create New Achievement Type
         if (this.Type === 'Create') {
            this.Form = new FormGroup({
               AchievementType: new FormControl( '', Validators.required),
               Institution: new FormControl(null, Validators.required ),
               Created_By: new FormControl( this.User_Id, Validators.required ),
            });
         }
      // If Edit New Achievement Type
         if (this.Type === 'Edit') {
            this.Form = new FormGroup({
               AchievementType: new FormControl(this.Data.AchievementType, Validators.required),
               AchievementType_Id: new FormControl(this.Data._id, Validators.required),
               Institution: new FormControl(null, Validators.required ),
   ...
   // Submit New Achievement Type
      submit() {
         if (this.Form.valid && !this.Uploading) {
            ...
            let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
            Info = Info.toString();
            this.Service.AchievementType_AsyncValidate({'Info': Info}).subscribe( response => {
               const ReceivingData = JSON.parse(response['_body']);
               if (response['status'] === 200 && ReceivingData['Status'] && ReceivingData['Available']) {

                  this.Service.AchievementType_Create({'Info': Info}).subscribe( response_1 => {
                     ...
                     if (response['status'] === 200 && ReceivingData_1.Status) {
                        ...
                        const DecryptedData = JSON.parse(CryptoBytes.toString(CryptoJS.enc.Utf8));
                        this.Toastr.NewToastrMessage( { Type: 'Success', Message: 'New Achievement Type Successfully Created' } );
                        ...
                        this.Toastr.NewToastrMessage( { Type: 'Error', Message: 'Error Not Identify!, Creating Achievement Type!' } );
   ...
   // Update Achievement Type
      update() {
         if (this.Form.valid && !this.Uploading) {
            ...
            let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
            Info = Info.toString();
            this.Service.AchievementTypeUpdate_AsyncValidate({'Info': Info}).subscribe( response => {
               const ReceivingData = JSON.parse(response['_body']);
               if (response['status'] === 200 && ReceivingData['Status'] && ReceivingData['Available']) {

                  this.Service.AchievementType_Update({'Info': Info}).subscribe( response_1 => {
                     this.Uploading = false;
                     const ReceivingData_1 = JSON.parse(response_1['_body']);
                     if (response['status'] === 200 && ReceivingData_1.Status) {
                        const CryptoBytes  = CryptoJS.AES.decrypt(ReceivingData_1['Response'], 'SecretKeyOut@123');
                        const DecryptedData = JSON.parse(CryptoBytes.toString(CryptoJS.enc.Utf8));
                        this.Toastr.NewToastrMessage( { Type: 'Success', Message: 'New Achievement Type Successfully Updated' } );
                        this.onClose.next({Status: true, Response: DecryptedData});
                        this.bsModalRef.hide();
                     } else if (response_1['status'] === 400 || response_1['status'] === 417 || response_1['status'] === 401  && !ReceivingData_1.Status) {
                        this.Toastr.NewToastrMessage( { Type: 'Error', Message: ReceivingData_1['Message'] } );
